...
create view testview as 
    ...
    from users u
    join profiles p
    on u.id = p.user_id and timestampdiff(year,p.birthday, now()) <= 20;
    
select * from testview; -- Для проверки
...
select 
    ...
    count(m.from_user_id) 
		as "amount of messages",
    dense_rank() 
    over(partition by "name" order by count(m.from_user_id) desc) as "rank"
from users u left join messages m
on u.id = m.from_user_id
group by u.id
order by "amount of messages";
...
select 
    ...
    created_at, -- если тут вписать lag(created_at,0) over(partition by "body"), результат останется тот же
    lead(created_at)
    over(partition by "body")) "time difference in minutes from the next message",
    timestampdiff(minute,
    lag(created_at)
    over(partition by "body"),
    created_at) -- если тут вписать lead(created_at,0) over(partition by "body"), результат останется тот же
    as "time difference in minutes from the previous message"
from messages
order by created_at;
